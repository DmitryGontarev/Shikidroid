package com.shikidroid.domain.models.common

/**
 * Типы жанра
 *
 * @property nameEng название жанра на английском
 * @property nameRu название жанра на русском
 * @property animeId идентификационный номер жанра для аниме
 * @property mangaId идентификационный номер жанра для манга
 */
enum class GenreType(
    val nameEng: String,
    val nameRu: String,
    val animeId: Long,
    val mangaId: Long
) {
    ACTION("action", "Экшен", 1, 56),
    ADVENTURE("adventure", "Приключения", 2, 68),
    CARS("cars", "Машины", 3, 84),
    COMEDY("comedy", "Комедия", 4, 49),
    DEMENTIA("dementia", "Безумие", 5, 90),
    DEMONS("demons", "Демоны", 6, 72),
    MYSTERY("mystery", "Детектив", 7, 46),
    DRAMA("drama", "Драма", 8, 50),
    ECCHI("ecchi", "Этти", 9, 51),
    FANTASY("fantasy", "Фэнтези", 10, 57),
    GAME("game", "Игры", 11, 79),
    HENTAI("hentai", "Хентай", 12, 59),
    HISTORICAL("historical", "Исторический", 13, 69),
    HORROR("horror", "Ужасы", 14, 80),
    KIDS("kids", "Детское", 15, 77),
    MAGIC("magic", "Магия", 16, 58),
    MARTIAL_ARTS("martial_arts", "Боевые исскуства", 17, 66),
    MECHA("mecha", "Меха", 18, 83),
    MUSIC("music", "Музыка", 19, 78),
    PARODY("parody", "Пародия", 20, 86),
    SAMURAI("samurai", "Самураи", 21, 88),
    ROMANCE("romance", "Романтика", 22, 62),
    SCHOOL("school", "Школа", 23, 60),
    SCI_FI("sci_fi", "Фантастика", 24, 53),
    SHOUJO("shoujo", "Сёдзе", 25, 63),
    SHOUJO_AI("shoujo_ai", "Сёдзе Ай", 26, 73),
    SHOUNEN("shounen", "Сёнен", 27, 47),
    SHOUNEN_AI("shounen_ai", "Сёнен Ай", 28, 55),
    SPACE("space", "Космос", 29, 85),
    SPORTS("sports", "Спорт", 30, 76),
    SUPER_POWER("super_power", "Супер силы", 31, 82),
    VAMPIRE("vampire", "Вампиры", 32, 64),
    YAOI("yaoi", "Яой", 33, 65),
    YURI("yuri", "Юри", 34, 75),
    HAREM("harem", "Гарем", 35, 71),
    SLICE_OF_LIFE("slice_of_life", "Повседневность", 36, 54),
    SUPERNATURAL("supernatural", "Сверхъестественное", 37, 48),
    MILITARY("military", "Военное", 38, 70),
    POLICE("police", "Полиция", 39, 89),
    PSYCHOLOGICAL("psychological", "Психологический", 40, 67),
    THRILLER("thriller", "Триллер", 41, 81),
    SEINEN("seinen", "Сейнен", 42, 52),
    JOSEI("josei", "Дзёсей", 43, 87),
    GENDER_BENDER("gender_bender", "Смена пола", -1, 74),
    DOUJINSHI("doujinshi", "Додзинси", -1, 61)
}

object GenreAlphabet {

    /** словарь с основными жанрами */
    val majorGenreList = listOf<GenreType>(
        GenreType.SHOUNEN,
        GenreType.SHOUNEN_AI,
        GenreType.SEINEN,
        GenreType.SHOUJO,
        GenreType.SHOUJO_AI,
        GenreType.JOSEI,
        GenreType.COMEDY,
        GenreType.ROMANCE,
        GenreType.SCHOOL,
        GenreType.SLICE_OF_LIFE,
        GenreType.MECHA
    )

    /** словарь с жанрами аниме в алфавитном порядке */
    val genreAnimeMap = mapOf<String, List<GenreType>>(
        "Б" to listOf(
            GenreType.DEMENTIA,
            GenreType.MARTIAL_ARTS
        ),
        "В" to listOf(
            GenreType.VAMPIRE,
            GenreType.MILITARY
        ),
        "Г" to listOf(GenreType.HAREM),
        "Д" to listOf(
            GenreType.DEMONS,
            GenreType.JOSEI,
            GenreType.MYSTERY,
            GenreType.KIDS,
            GenreType.DRAMA
        ),
        "И" to listOf(
            GenreType.GAME,
            GenreType.HISTORICAL
        ),
        "К" to listOf(
            GenreType.COMEDY,
            GenreType.SPACE
        ),
        "М" to listOf(
            GenreType.MAGIC,
            GenreType.CARS,
            GenreType.MECHA,
            GenreType.MUSIC
        ),
        "П" to listOf(
            GenreType.PARODY,
            GenreType.SLICE_OF_LIFE,
            GenreType.POLICE,
            GenreType.ADVENTURE,
            GenreType.PSYCHOLOGICAL
        ),
        "Р" to listOf(GenreType.ROMANCE),
        "С" to listOf(
            GenreType.SAMURAI,
            GenreType.SUPERNATURAL,
            GenreType.SEINEN,
            GenreType.SHOUJO,
            GenreType.SHOUJO_AI,
            GenreType.SHOUNEN,
            GenreType.SHOUNEN_AI,
            GenreType.SPORTS,
            GenreType.SUPER_POWER
        ),
        "T" to listOf(GenreType.THRILLER),
        "У" to listOf(GenreType.HORROR),
        "Ф" to listOf(
            GenreType.SCI_FI,
            GenreType.FANTASY
        ),
        "Х" to listOf(GenreType.HENTAI),
        "Ш" to listOf(GenreType.SCHOOL),
        "Э" to listOf(
            GenreType.ACTION,
            GenreType.ECCHI
        ),
        "Ю" to listOf(GenreType.YURI),
        "Я" to listOf(GenreType.YAOI)
    )

    /** словарь с жанрами аниме в алфавитном порядке с цензурой */
    val genreAnimeCensoredMap = mapOf<String, List<GenreType>>(
        "Б" to listOf(
            GenreType.DEMENTIA,
            GenreType.MARTIAL_ARTS
        ),
        "В" to listOf(
            GenreType.VAMPIRE,
            GenreType.MILITARY
        ),
        "Г" to listOf(GenreType.HAREM),
        "Д" to listOf(
            GenreType.DEMONS,
            GenreType.JOSEI,
            GenreType.MYSTERY,
            GenreType.KIDS,
            GenreType.DRAMA
        ),
        "И" to listOf(
            GenreType.GAME,
            GenreType.HISTORICAL
        ),
        "К" to listOf(
            GenreType.COMEDY,
            GenreType.SPACE
        ),
        "М" to listOf(
            GenreType.MAGIC,
            GenreType.CARS,
            GenreType.MECHA,
            GenreType.MUSIC
        ),
        "П" to listOf(
            GenreType.PARODY,
            GenreType.SLICE_OF_LIFE,
            GenreType.POLICE,
            GenreType.ADVENTURE,
            GenreType.PSYCHOLOGICAL
        ),
        "Р" to listOf(GenreType.ROMANCE),
        "С" to listOf(
            GenreType.SAMURAI,
            GenreType.SUPERNATURAL,
            GenreType.SEINEN,
            GenreType.SHOUJO,
            GenreType.SHOUJO_AI,
            GenreType.SHOUNEN,
            GenreType.SHOUNEN_AI,
            GenreType.SPORTS,
            GenreType.SUPER_POWER
        ),
        "T" to listOf(GenreType.THRILLER),
        "У" to listOf(GenreType.HORROR),
        "Ф" to listOf(
            GenreType.SCI_FI,
            GenreType.FANTASY
        ),
        "Ш" to listOf(GenreType.SCHOOL),
        "Э" to listOf(
            GenreType.ACTION,
            GenreType.ECCHI
        )
    )

    /** словарь с жанрами манги/ранобэ в алфавитном порядке */
    val genreMangaRanobeMap = mapOf<String, List<GenreType>>(
        "Б" to listOf(
            GenreType.DEMENTIA,
            GenreType.MARTIAL_ARTS
        ),
        "В" to listOf(
            GenreType.VAMPIRE,
            GenreType.MILITARY
        ),
        "Г" to listOf(GenreType.HAREM),
        "Д" to listOf(
            GenreType.DEMONS,
            GenreType.JOSEI,
            GenreType.MYSTERY,
            GenreType.KIDS,
            GenreType.DOUJINSHI,
            GenreType.DRAMA
        ),
        "И" to listOf(
            GenreType.GAME,
            GenreType.HISTORICAL
        ),
        "К" to listOf(
            GenreType.COMEDY,
            GenreType.SPACE
        ),
        "М" to listOf(
            GenreType.MAGIC,
            GenreType.CARS,
            GenreType.MECHA,
            GenreType.MUSIC
        ),
        "П" to listOf(
            GenreType.PARODY,
            GenreType.SLICE_OF_LIFE,
            GenreType.POLICE,
            GenreType.ADVENTURE,
            GenreType.PSYCHOLOGICAL
        ),
        "Р" to listOf(GenreType.ROMANCE),
        "С" to listOf(
            GenreType.SAMURAI,
            GenreType.SUPERNATURAL,
            GenreType.SEINEN,
            GenreType.SHOUJO,
            GenreType.SHOUJO_AI,
            GenreType.SHOUNEN,
            GenreType.SHOUNEN_AI,
            GenreType.GENDER_BENDER,
            GenreType.SPORTS,
            GenreType.SUPER_POWER
        ),
        "T" to listOf(GenreType.THRILLER),
        "У" to listOf(GenreType.HORROR),
        "Ф" to listOf(
            GenreType.SCI_FI,
            GenreType.FANTASY
        ),
        "Х" to listOf(GenreType.HENTAI),
        "Ш" to listOf(GenreType.SCHOOL),
        "Э" to listOf(
            GenreType.ACTION,
            GenreType.ECCHI
        ),
        "Ю" to listOf(GenreType.YURI),
        "Я" to listOf(GenreType.YAOI)
    )

    /** словарь с жанрами манги/ранобэ в алфавитном порядке с цензурой */
    val genreMangaRanobeCensoredMap = mapOf<String, List<GenreType>>(
        "Б" to listOf(
            GenreType.DEMENTIA,
            GenreType.MARTIAL_ARTS
        ),
        "В" to listOf(
            GenreType.VAMPIRE,
            GenreType.MILITARY
        ),
        "Г" to listOf(GenreType.HAREM),
        "Д" to listOf(
            GenreType.DEMONS,
            GenreType.JOSEI,
            GenreType.MYSTERY,
            GenreType.KIDS,
            GenreType.DOUJINSHI,
            GenreType.DRAMA
        ),
        "И" to listOf(
            GenreType.GAME,
            GenreType.HISTORICAL
        ),
        "К" to listOf(
            GenreType.COMEDY,
            GenreType.SPACE
        ),
        "М" to listOf(
            GenreType.MAGIC,
            GenreType.CARS,
            GenreType.MECHA,
            GenreType.MUSIC
        ),
        "П" to listOf(
            GenreType.PARODY,
            GenreType.SLICE_OF_LIFE,
            GenreType.POLICE,
            GenreType.ADVENTURE,
            GenreType.PSYCHOLOGICAL
        ),
        "Р" to listOf(GenreType.ROMANCE),
        "С" to listOf(
            GenreType.SAMURAI,
            GenreType.SUPERNATURAL,
            GenreType.SEINEN,
            GenreType.SHOUJO,
            GenreType.SHOUJO_AI,
            GenreType.SHOUNEN,
            GenreType.SHOUNEN_AI,
            GenreType.GENDER_BENDER,
            GenreType.SPORTS,
            GenreType.SUPER_POWER
        ),
        "T" to listOf(GenreType.THRILLER),
        "У" to listOf(GenreType.HORROR),
        "Ф" to listOf(
            GenreType.SCI_FI,
            GenreType.FANTASY
        ),
        "Ш" to listOf(GenreType.SCHOOL),
        "Э" to listOf(
            GenreType.ACTION,
            GenreType.ECCHI
        )
    )
}